...
use crate::{
    blueprint::{
        CircuitEntity, Combinator, ConnectionPoint, ConnectionPointKind, ConstantCombinator,
        CoordSet, Graph, Operation,
    },
...
impl super::Placer for TurdMaster2000 {
    fn place(graph: Graph) -> Vec<fbo::Entity> {
        let mut placer = TurdMaster2000::new(graph);

        // Place input combinators
        for (n, (name, input_nid, _sig)) in
            placer.graph.clone().get_input_nodes().iter().enumerate()
        {
            let x = (n / 2) as i64;
            let y = (n % 2) as i64;
            let pos: CoordSet = (r64(x as f64), r64(y as f64));
            let en = placer.get_next_entity_number();
            let const_com = ConstantCombinator::new_sign(name, en, *input_nid, pos);
            placer.placed_entities.insert(en, Box::new(const_com));
            placer.placed_positions.insert((x, y));
            placer.max_x = cmp::max(placer.max_x, x);
            placer.max_y = cmp::max(placer.max_y, y);
        }

        // Place output combinators
        for (n, (name, output_nid)) in placer.graph.get_output_nodes().iter().enumerate() {
            let x = (n / 2) as i64 + placer.max_x + 1;
            let y = (n % 2) as i64;
            let pos: CoordSet = (r64(x as f64), r64(y as f64));
            let en = placer.get_next_entity_number();
            ...
            for y in placer.min_y - 1..=placer.max_y + 1 {
                for x in placer.min_x - 1..=placer.max_x + 1 {
                    if let Some(combinator) =
                        placer.try_place_combinator(x, y, input_nid, output_nid, &operation)
                    {
                        placer
                            .placed_entities
